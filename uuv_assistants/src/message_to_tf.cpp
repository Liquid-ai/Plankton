...
//   g_tf_prefix ;//= tf2::getPrefixParam(priv_nh); //to check
//   g_transform_broadcaster = new tf2_ros::StaticTransformBroadcaster(node); //to check
...
//   }
//   if (!g_topic.empty()) {
//       RCLCPP_FATAL(node->get_logger(), "Not working anymore, I broke it.");
...
//     // else
//     //   g_pose_publisher = priv_nh.advertise<geometry_msgs::msg::PoseStamped>("pose", 10); //to check
//   }
...
//     // else
//     //   g_euler_publisher = priv_nh.advertise<geometry_msgs::msg::Vector3Stamped>("euler", 10); //to check
//   }
...
int main(int argc, char** argv) {
  ...
  g_tf_prefix = tf2::getPrefixParam(priv_nh); // TODO Check
  ...
  if (publish_pose) {
    ...
    if (!publish_pose_topic.empty())
      g_pose_publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>(publish_pose_topic, 10);
    // else
    //   g_pose_publisher = priv_nh.advertise<geometry_msgs::msg::PoseStamped>("pose", 10); //TODO to check
  }
